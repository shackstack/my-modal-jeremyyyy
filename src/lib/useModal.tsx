...
const ModalBackDrop = styled.div<{ modalOpen: boolean; isLoading: boolean }>`
  ...
  visibility: ${({ modalOpen, isLoading }) =>
    modalOpen ? 'visible' : isLoading ? 'visible' : 'hidden'};
  transition: visibility ${({ modalOpen }) => (modalOpen ? '0s' : '0.4s')}
    ease-out;
`;
...
const ModalContent = styled.div<{ modalOpen: boolean }>`
  ...
  animation: ${({ modalOpen }) => (modalOpen ? slideUp : slideDown)} 0.4s
    ease-out;
`;
